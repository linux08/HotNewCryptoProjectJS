...
class Twitter {
  getFollowers(userId) {
    return new Promise((res, rej) => {
      twitterClient.get("followers/list", { screen_name: userId }, function (err, data, response) {
        if (err) rej(err);
        return res(data);
      });
    });
      ...
      try {
        ...
        for (let i = 0; i < user.users.length; i++) {
          ...
          if (!dbUser.length) {
            ...
            currentFriend = await User.findOneAndUpdate(
              { id_str: currentUser.id_str },
